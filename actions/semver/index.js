...
async function main() {
    const hotfixPrefix = core.getInput('hotfix-prefix')
    ...
    const maxTagsToFetch = _.toSafeInteger(
        core.getInput('maxTagsToFetch') || 10)
    const fetchLimit = (maxTagsToFetch < 1 || maxTagsToFetch > 100) ? 10
        : maxTagsToFetch

    const bumpTypes = {
        major: core.getInput('majorList').split(',').map(p => p.trim()).filter(
            p => p),
        minor: core.getInput('minorList').split(',').map(p => p.trim()).filter(
            p => p),
        patch: core.getInput('patchList').split(',').map(p => p.trim()).filter(
            p => p),
        patchAll: (core.getInput('patchAll') === true || core.getInput(
            'patchAll') === 'true')
    }
    ...
    `, {
            owner, repo, tag: `refs/tags/${prefix}${fromTag}`
        })
    ...
    for (const commit of commits) {
        try {
            ...
            } else if (bumpTypes.patchAll || bumpTypes.patch.includes(
                cAst.type)) {
    ...
    if (majorChanges.length > 0) {
        ...
        switch (noVersionBumpBehavior) {
            ...
            case 'major': {
                core.info(
                    'No commit resulted in a version bump since last release! Defaulting to using MAJOR...')
                bump = 'major'
                break
            }
            case 'minor': {
                core.info(
                    'No commit resulted in a version bump since last release! Defaulting to using MINOR...')
                bump = 'minor'
                break
            }
            case 'patch': {
    ...
    if (branch.includes(hotfixPrefix)) {
        next = semver.inc(latestTag.name, 'pre', 'hotfix');
    }
    else {
        next = semver.inc(semver.coerce(latestTag.name), bump)
    }
